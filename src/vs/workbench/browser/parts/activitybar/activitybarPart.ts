...
export class ActivitybarPart extends Part {
	...
	get minimumWidth(): number {
		return this._isCompact
			? ActivitybarPart.COMPACT_ACTIVITYBAR_WIDTH
			: ActivitybarPart.ACTIVITYBAR_WIDTH;
	}
	...
	private updateCompactStyle(): void {
		if (this.element) {
			...
			this.element.style.setProperty(
				'--activity-bar-width',
				`${this.minimumWidth}px`,
			);
			this.element.style.setProperty(
				'--activity-bar-action-height',
				`${this._isCompact ? ActivitybarPart.COMPACT_ACTION_HEIGHT : ActivitybarPart.ACTION_HEIGHT}px`,
			);
			this.element.style.setProperty(
				'--activity-bar-icon-size',
				`${this._isCompact ? ActivitybarPart.COMPACT_ICON_SIZE : ActivitybarPart.ICON_SIZE}px`,
			);
		}
	}
	...
	private createCompositeBar(): PaneCompositeBar {
		const actionHeight = this._isCompact
			...
			: ActivitybarPart.ACTION_HEIGHT;
		const iconSize = this._isCompact
			? ActivitybarPart.COMPACT_ICON_SIZE
...
export class ActivityBarCompositeBar extends PaneCompositeBar {
	...
	private readonly menuBar = this._register(
		new MutableDisposable<CustomMenubarControl>(),
	);
	...
	private installMenubar() {
		...
		this.menuBar.value = this._register(
	...
	override layout(width: number, height: number): void {
		...
		if (this.globalCompositeBar) {
			if (this.options.orientation === ActionsOrientation.VERTICAL) {
				height -=
					this.globalCompositeBar.size() * this.options.overflowActionSize;
			} else {
	...
	getActivityBarContextMenuActions(): IAction[] {
		...
		const positionActions = getContextMenuActions(
		...
		if (this.part === Parts.SIDEBAR_PART) {
			actions.push(
				toAction({
					...
					run: () =>
						this.instantiationService.invokeFunction((accessor) =>
							new ToggleSidebarVisibilityAction().run(accessor),
						),
				}),
			);
		}
