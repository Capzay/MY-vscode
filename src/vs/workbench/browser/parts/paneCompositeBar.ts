...
import { ICompositeDropdownItem } from './compositeBarDropdownAction.js';
import {
...
export class PaneCompositeBar extends Disposable {
	...
	private createCompositeBar(cachedItems: ICompositeBarItem[]) {
		return this._register(
			this.instantiationService.createInstance(CompositeBar, cachedItems, {
				...
				getDropdownItems: () => this.getAllViewContainersForDropdown(),
	...
	private getContextMenuActionsForComposite(compositeId: string): IAction[] {
		...
		for (const location of [
			...
			if (currentLocation !== location) {
				moveActions.push(
					this.createMoveAction(viewContainer, location, defaultLocation),
				);
			}
		}
		...
		if (defaultLocation !== currentLocation) {
			...
			if (viewContainerModel.allViewDescriptors.length === 1) {
				...
				const defaultContainer =
					this.viewDescriptorService.getDefaultContainerById(viewToReset.id)!;
	...
	private createMoveAction(
		...
		newLocation: ViewContainerLocation,
		defaultLocation: ViewContainerLocation,
	): IAction {
		return toAction({
			id: `moveViewContainerTo${newLocation}`,
			label:
				newLocation === ViewContainerLocation.Panel
					? localize("panel", "Panel")
					: newLocation === ViewContainerLocation.Sidebar
						? localize("sidebar", "Primary Side Bar")
						: localize("auxiliarybar", "Secondary Side Bar"),
			run: () => {
				let index: number | undefined;
				if (newLocation !== defaultLocation) {
					index =
						this.viewDescriptorService.getViewContainersByLocation(
							newLocation,
						).length; // move to the end of the location
				} else {
	...
	getVisiblePaneCompositeIds(): string[] {
		return this.compositeBar
			...
			.filter(
				(v) =>
					this.paneCompositePart.getActivePaneComposite()?.getId() === v.id ||
	...
	private getAllViewContainersForDropdown(): ICompositeDropdownItem[] {
		return this.compositeBar.getCompositeBarItems().map((item) => {
			const viewContainer = this.getViewContainer(item.id);
			const viewContainerModel = viewContainer
				? this.viewDescriptorService.getViewContainerModel(viewContainer)
				: undefined;
			const keybindingId = viewContainerModel?.keybindingId;
			const keybinding = keybindingId
				? this.keybindingService.lookupKeybinding(keybindingId)
				: undefined;
			...
			return {
				...
				icon: viewContainerModel?.icon
					? this.getIconClassNames(viewContainerModel.icon)
					: undefined,
	...
	private updateCompositeBarItemsFromStorage(retainExisting: boolean): void {
		...
		if (retainExisting) {
			for (const compositeItem of compositeItems) {
				const newCompositeItem = newCompositeItems.find(
					({ id }) => id === compositeItem.id,
				);
				if (!newCompositeItem) {
			...
			cachedViewContainers.map(
				({ id, pinned, order }) =>
					({
						...
						pinned,
						visible: Boolean(
							pinnedViewContainers.find(({ id: pinnedId }) => pinnedId === id)
								?.visible,
						),
						order,
	...
	private set viewContainersWorkspaceStateValue(
		viewContainersWorkspaceStateValue: string,
	) {
		if (
...
class ViewContainerActivityAction extends CompositeBarAction {
	...
	private updateActivity(): void {
		this.activities = this.activityService.getViewContainerActivities(
